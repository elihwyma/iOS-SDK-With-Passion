/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/BehaviorMiner.framework/BehaviorMiner
 */

@interface BMRetrievalFilter : NSObject {
    long long  _inclusionOperator;
    struct NSSet { Class x1; } * _items;
    long long  _operand;
    NSSet * _types;
}

@property (nonatomic) long long inclusionOperator;
@property (nonatomic, retain) NSSet *items;
@property (nonatomic) long long operand;
@property (nonatomic, retain) NSSet *types;

+ (id)filterWithOperand:(long long)arg1 inclusionOperator:(long long)arg2 items:(struct NSSet { Class x1; }*)arg3;
+ (id)filterWithOperand:(long long)arg1 inclusionOperator:(long long)arg2 types:(id)arg3;

- (void).cxx_destruct;
- (long long)inclusionOperator;
- (struct NSSet { Class x1; }*)items;
- (long long)operand;
- (id)rulePredicate;
- (void)setInclusionOperator:(long long)arg1;
- (void)setItems:(struct NSSet { Class x1; }*)arg1;
- (void)setOperand:(long long)arg1;
- (void)setTypes:(id)arg1;
- (id)types;

@end
