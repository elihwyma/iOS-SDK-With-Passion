/*
 Image: /System/Library/PrivateFrameworks/CloudDocsUI.framework/CloudDocsUI
 */

#import <CloudDocsUI/Swift-Protocol.h>

@protocol UIContentContainer <Swift>

@property (nonatomic, readonly) struct CGSize preferredContentSize;

- (unsigned short)willTransitionToTraitCollection:withTransitionCoordinator: /* Error: Ran out of types for this method. */;
- (unsigned short)preferredContentSizeDidChangeForChildContentContainer: /* Error: Ran out of types for this method. */;
- (unsigned short)viewWillTransitionToSize:withTransitionCoordinator: /* Error: Ran out of types for this method. */;
- (unsigned short)systemLayoutFittingSizeDidChangeForChildContentContainer: /* Error: Ran out of types for this method. */;
- (unsigned short)sizeForChildContentContainer:withParentContainerSize: /* Error: Ran out of types for this method. */;

@end
